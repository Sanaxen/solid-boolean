...
int main(int argc, char **argv) {
  ...
  if(!(data = malloc(num_pts))) {
	...
	pnts = (double*)malloc(3*num_pts*sizeof(double));
	pppp = pnts;
	kdGetPoint = getPnt;
  ...
  printf("ok.\n");
  //kd_insert3( ptree, pt[0]+1, pt[1], pt[2], "aaaa");

  /* find points closest to the origin and within distance radius */
  presults = kd_nearest_range( (struct kdtree *)ptree, pt, radius );
  ...
  while( !kd_res_end( presults ) ) {
    ...
    pch = (char*)kd_res_item( presults, &ipos );

	pos[0] = kdGetPoint(ipos, 0);
	pos[1] = kdGetPoint(ipos, 1);
	pos[2] = kdGetPoint(ipos, 2);
